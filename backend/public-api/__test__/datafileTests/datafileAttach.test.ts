...
describe("Checks if /attach for DataFile works", () => {
  let app: Application;
  ...
  it('Should return {"status":"200"} for TXT file', async () => {
    const response = await request(app)
      .post(`/api/datafile/${id}/attach`)
      .field("fileType", "TXT")
  ...
  it('Should return {"status":"200"} for JSON file', async () => {
    const response = await request(app)
      .post(`/api/datafile/${id}/attach`)
      .field("fileType", "JSON")
  ...
  it('Should return {"status":"400"} for Referenced Datafile', async () => {
    const response = await request(app)
